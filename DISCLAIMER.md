...
**This software provides general information and examples only. It is not legal advice.**
...
- When a language model is enabled, its statements are only shown if their quotes are found verbatim in the retrieved provisions, but the model can still misread a provision, omit an exception or draw a conclusion the text does not support.
